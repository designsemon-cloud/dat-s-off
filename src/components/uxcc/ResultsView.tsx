...
export function ResultsView({ imageUrl, imageName, stack, device, result, onReset }: ResultsViewProps) {
  const exportPDF = () => {
  ...
  return (
    ...
    >
      <AppHeader onNewAnalysis={onReset} credits={4} />

      <div className="mx-auto w-full max-w-7xl px-6 py-6">
        {/* Horizontal split: left = image + details, right = actions + fixes */}
            ...
            <div className="rounded-2xl border border-border bg-bg-tertiary/60 p-4 backdrop-blur">
              <div className="flex items-center gap-3 mb-3">
                <div className="grid h-11 w-11 place-items-center rounded-xl bg-primary text-primary-foreground">
                  <span className="text-base font-bold">{result.score}</span>
                </div>
                <div>
                  <div className="text-[10px] font-medium uppercase tracking-wider text-text-tertiary">UX Score</div>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                ...
                <Stat label="Critical" value={`${result.fixes.filter((f) => f.severity === "high").length}`} tone="text-severity-high" />
              </div>
            </div>
          </motion.aside>
          ...
          >
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-bg-tertiary/60 p-4 backdrop-blur">
              <div>
                ...
                <p className="text-xs text-text-tertiary mt-0.5">{result.fixes.length} prioritized fixes ready</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <GlowButton variant="outline" size="sm" onClick={exportPDF}>
                  <Download className="h-3.5 w-3.5" /> Export PDF
                </GlowButton>
                <GlowButton variant="outline" size="sm" onClick={() => {
                  const md = `# UXCC Report\n\nScore: ${result.score}/100\n\n${result.summary}\n\n` +
                    result.fixes.map((f, i) => `## ${i + 1}. ${f.title} [${f.severity}]\n**${f.principle}**\n\n${f.problem}\n\n*Why:* ${f.why}\n\n*Fix:* ${f.solution}\n\n\`\`\`${f.language}\n${f.code}\n\`\`\``).join("\n\n");
...
function Stat({ label, value, tone = "text-foreground" }: { label: string; value: string; tone?: string }) {
  return (
    <div className="rounded-lg border border-border bg-bg-secondary/60 p-2.5">
      ...
      <div className={`mt-1 text-sm font-semibold capitalize ${tone}`}>{value}</div>
    </div>
  );
}
